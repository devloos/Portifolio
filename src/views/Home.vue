<template>
  <main class="container flex max-w-4xl flex-col justify-between">
    <div
      class="flex flex-col items-center justify-center gap-4 pt-8 md:flex-row-reverse md:gap-2 md:px-3 md:pt-20"
    >
      <img
        class="mx-auto max-w-[150px] self-center rounded-full border border-slate-700"
        src="stud.jpg"
        alt="stud"
      />
      <div class="grow px-2 text-center md:text-start">
        <p class="mb-6 max-w-xl px-1 text-2xl font-semibold md:text-3xl">
          Hey! My name is Carlos. I work as a Web Developer at Ethika Inc.
        </p>
        <div class="px-2">
          <button
            class="mr-6 rounded bg-alternate-light px-5 py-2 ring-2 ring-gray-500 transition-all hover:bg-alternate dark:bg-alternate-dark dark:ring-alternate hover:dark:bg-alternate"
          >
            <div class="flex items-center justify-center gap-3">
              <p>Resume</p>
              <i class="fa-solid fa-download"></i>
            </div>
          </button>
          <button
            class="rounded bg-slate-300 px-5 py-2 ring-2 ring-slate-400 dark:bg-slate-700 dark:ring-slate-500"
          >
            <div class="flex items-center justify-center gap-3">
              <p>Projects</p>
              <i class="fa-solid fa-arrow-right"></i>
            </div>
          </button>
        </div>
      </div>
    </div>
    <div id="contact">dkjlj</div>
  </main>
</template>
